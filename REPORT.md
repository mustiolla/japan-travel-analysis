...
단변량 상관분석 및 시차 분석에서 도출된 개별 결론들을 통계적으로 한 번에 통합 검증하기 위해, **'환율 3개월 시차(Lag 3)'와 '월별 계절성(더미 변수)'을 결합한 다변량 OLS(Ordinary Least Squares) 회귀분석**을 수행했다.

* **회귀 모형:** $\text{Visitors} = \beta_0 + \beta_1 \cdot \text{Exchange\_Rate\_Lag3} + \sum_{m=2}^{12} \gamma_m \cdot D_m + \epsilon$ (기준월: 1월)
* **주요 검증 결과:**
  * **설명력 ($R^2$):** **0.452 (수정된 $R^2 = 0.389$, $F\text{-statistic } p\text{-value} = 2.27 \times 10^{-9}$)**  
    $\rightarrow$ 거시 환율 시차와 12개 월 더미 변수만으로도 한국인 방일 관광객 변동의 **45.2%를 유의미하게 설명**함.
  * **환율 3개월 시차 계수 ($\beta_1$):** **$-68,310.8$ ($p\text{-value} = 4.568 \times 10^{-14}$)**  
    $\rightarrow$ 1엔당 환율이 1원 하락(원화 가치 상승/엔저 심화)할 때마다 3개월 뒤 방문객 수는 **평균 약 6만 8천 명 증가**하는 극도로 유의미한 경제적 파급효과 입증.
  * **월별 계절성 계수 분석:** 기준월인 1월 대비 모든 월의 계수가 음수로 나타났으며, 특히 **9월($-169,100$, $p=0.015$)**의 감소폭이 연중 가장 컸음.  
    $\rightarrow$ 앞서 박스플롯 분석에서 규명한 '1~2월 최대 성수기' 및 '9월 최비수기' 패턴이 통계적 회귀 계수로도 완벽히 교차 입증됨.
...
**1. 환율과 여행 수요의 '3~4개월 골든타임' 및 정량 모델링**
전반적으로 원/엔 환율이 하락하면 방문객이 증가하는 확고한 역의 상관관계가 존재한다. 특히 당월의 즉각적인 반응보다 **환율 하락 시점으로부터 3~4개월 뒤에 수요가 가장 폭발적으로 반응하는 '지연 효과(Lag Effect)'**를 통계적으로 입증했다. 나아가 **다변량 OLS 회귀분석을 통해 환율 3개월 시차와 월별 계절성만으로 수요 변동의 45.2%($R^2 = 0.452$)를 설명**할 수 있으며, 1엔당 환율 1원 하락 시 3개월 뒤 방문객이 평균 약 6.8만 명 증가한다는 구체적인 정량적 산출 기준을 확립했다. 또한 환율 상황과 무관하게 매년 1~2월(겨울 방학)은 절대적인 수요를 유지하는 뚜렷한 계절성(Seasonality)을 띠고 있다.
...
| 다변량 OLS 회귀분석 (환율 시차 + 월 더미) | 채택 | 시차와 계절성의 복합 설명력($R^2=0.452$) 및 환율 1원당 방문객 변동 정량화 |
