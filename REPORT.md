...
5. **[심층 분석 2]** 대지진, 전염병, 국가적 갈등 등 역사적 외부 충격(Black Swan) 발생 시, 환율과 여행 수요 간의 경제적 상관관계는 어떻게 변화하는가?
...
* **관찰(Fact):** 당월(0개월 차)에도 -0.596으로 유의미한 역의 상관관계가 나타나지만, 시간이 지날수록 상관성이 점진적으로 강해져 **3개월 차와 4개월 차에 공통으로 최고점(-0.632)을 기록**했다.
...
## ⚠️ 분석의 한계점 및 향후 과제 (Limitations & Future Work)

본 분석은 거시적 지표(환율, 전체 방문객 수)를 바탕으로 유의미한 비즈니스 인사이트를 도출했으나, 다음과 같은 한계가 존재하며 이는 향후 데이터 확장을 통해 보완할 수 있다.

* **지출액(질적 데이터)의 부재:** 단순히 일본에 입국한 '사람 수(양적 데이터)'만 분석했을 뿐, 엔저 현상이 실제 관광객의 현지 체류일수 증가나 쇼핑 등 '1인당 관광 지출액' 확대로 이어졌는지는 본 데이터만으로 증명하기 어렵다.
* **인구통계학적 특성 미반영:** 연령대, 성별, 동반자 형태(가족 vs 1인)에 따라 환율 하락에 반응하는 민감도가 다를 수 있으나, 전체 합계 데이터만 사용하여 초개인화된 세부 타겟팅 전략을 세우는 데는 한계가 있다.
* **공급 측면의 제약 변수 누락:** 항공사의 일본 노선 운항 편수나 좌석 공급량, 현지 호텔의 객실 수라는 물리적 제약이 실제 여행객 수를 결정짓는 중요한 요소임에도 불구하고, 본 모델에서는 수요 측면의 지표만 주로 고려되었다.
