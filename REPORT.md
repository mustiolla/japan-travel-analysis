...
* **Q3. 솟구친 '보라색 봉우리(Peak, $r > 0$)'는 무엇을 증명하나요?**
  * 평상시에는 보라색 선이 바닥($-0.5 \sim -0.8$)에 머물며 정상적인 역관계를 유지하다가, **충격 사건 전후로 0을 뚫고 하늘로 치솟는 현상($r > 0$)**이 관찰됩니다.
  * 이는 환율(가격)이라는 경제적 인센티브가 아무리 강력해도, **'생명 안전 위협(대지진/원전/전염병)'이나 '국가적 정서/갈등(세월호 애도/노재팬 불매)'이 터지면 경제 법칙이 무력화된다는 사실**을 보여줍니다.
* **Q4. 사건 직후 왜 '양(+)의 봉우리'로 가지 않고 '음(-)의 값'으로 내려가는 구간이 있나요? (핵심 분석)**
  * **1) [노재팬 & 코로나] 환율 급등(엔고)과 여행객 폭락이 겹쳐 통계상 강력한 음수가 됨:**
    - 2019년 7월 노재팬 불매운동 당시, 무역 분쟁 여파로 원/엔 환율은 **1,000원 $\rightarrow$ 1,138원으로 급등(엔고, $\uparrow$)**했고, 여행객은 불매운동으로 **월 56만 명 $\rightarrow$ 20만 명으로 폭락($\downarrow$)**했습니다.
    - 두 변수가 서로 정반대($\text{환율 } \uparrow, \text{여행객 } \downarrow$)로 움직였기 때문에, 통계 공식상 "환율이 올라서 여행객이 줄어들었다"는 **극도로 강한 음(-)의 상관관계($r = -0.735$)**로 계산된 것입니다. (코로나19 역시 국경 봉쇄와 안전자산 엔고가 겹쳐 강한 음수 유지)
  * **2) [세월호 & 구마모토 지진] 2~3개월 단기 충격 후 '엔저 호황'으로 즉시 정상 복귀:**
    - 사건 발생 전후 일시적으로 양수 피크($+0.7$)를 친 뒤, 2014년 하반기(아베노믹스 엔저) 및 2016년 하반기(LCC 증편)에 여행객이 폭증하면서 시장이 **정상적인 음(-)의 경제 법칙으로 급속히 회귀**했기 때문입니다.
  * **3) 12개월 이동창(Rolling Window)의 시간차 지연:**
    - 과거 12개월 데이터가 묶여 계산되므로, 사건의 영향이 윈도우에 진입하고 빠져나가는 과정에서 봉우리의 위치가 3~6개월 앞뒤로 시차를 두고 형성됩니다.
...
* **주요 검증 결과:**
  ...
  * **환율 3개월 시차 계수 ($\beta_1$):** **$-68,310.8$ ($p\text{-value} = 4.568 \times 10^{-14}$)**  
    $\rightarrow$ 수집된 데이터의 독립변수 단위는 **원/1엔**(`JPYKRW=X`)입니다. 따라서 **1엔당 1원 하락(일반적인 뉴스 고시 기준 '100엔당 100원 하락')할 때마다 3개월 뒤 방문객 수는 평균 약 6만 8,311명(약 6.8만 명) 증가**합니다.  
    *(※ 일상적인 100엔 환율 기준으로 환산하면, **100엔당 1원 하락할 때마다 약 683명 증가**하는 매우 합리적이고 상식적인 수치임)*
...
| **웹 대시보드 서비스화** | **채택** | `Streamlit` + `Plotly` 기반 인터랙티브 대시보드(`app.py`) 구축으로 기간/조건별 실시간 탐색 제공 |
| **시계열 베이스라인 예측** | **채택** | Holt-Winters 지수평활법으로 향후 6개월 단기 수요를 예측하고 가정 및 한계점을 균형 있게 명시 |

---

## 8. [보너스 과제] 분석 결과 서비스화 및 시계열 심화 옵션

### 8.1 분석 결과 서비스화: Streamlit 인터랙티브 웹 대시보드 (`app.py`)
- **개발 목적**: 정적인 리포트의 한계를 넘어, 심사자와 사용자가 직접 **기간(Date Range)**과 **조건(Lag, 역사적 사건 오버레이, 이동평균선)**을 변경하며 가설을 인터랙티브하게 검증할 수 있도록 서비스화하였습니다.
- **핵심 기술**: `Streamlit`, `Plotly`, `Statsmodels`
- **구현 기능**:
  1. **사이드바 제어판**:
     - 분석 기간 프리셋 (전체 기간 2010~현재, 코로나 이전 황금기 2010~2019, 코로나 충격 및 회복기 2020~현재, 사용자 직접 슬라이더)
     - 환율 시차(Lag 0~6개월) 실시간 조정 슬라이더
     - 6대 역사적 충격(Black Swan) 수직선 및 주석 On/Off 토글
     - 3MA / 12MA 이동평균선 On/Off 토글
  2. **5개 메인 탭 구성**:
     - **탭 1 (시계열 동향 & 이중 축)**: 방일객과 원/엔 환율 이중 축 인터랙티브 차트 및 12개월 이동 상관계수 추이
     - **탭 2 (시차 및 회귀 시뮬레이션)**: 선택된 Lag에 따른 산점도, 실시간 OLS 회귀선 및 시차별 상관계수 비교 막대차트
     - **탭 3 (시계열 분해 - 옵션 A)**: 추세, 계절성, 잔차 4단 인터랙티브 분해 차트 및 월별 계절성 박스플롯
     - **탭 4 (베이스라인 단기 예측 - 옵션 B)**: 향후 6~12개월 예측치, 80%/95% 신뢰구간 밴드 및 가정/한계 설명 패널
     - **탭 5 (핵심 인사이트 & 가이드)**: 비즈니스 의사결정 규칙 및 정책 제언 요약
- **제출 산출물**:
  - `DASHBOARD_GUIDE.md`: 로컬 실행 명령어 안내(제출 옵션 2) + 필터/기간 변경 시나리오별 상세 설명서(제출 옵션 3)

---

### 8.2 시계열 심화 옵션: 베이스라인 단기 예측 (Holt-Winters) 및 가정/한계 분석

본 과제에서는 단순 예측 정확도 경쟁을 지양하고, **"통계 모델이 성립하기 위한 전제 가정(Assumptions)"**과 **"실제 현실과의 괴리로 인한 구조적 한계(Limitations)"**를 명확히 밝히는 데 집중하였습니다.

#### 1) 베이스라인 예측 모델 개요
- **적용 모델**: Holt-Winters Exponential Smoothing (가법 추세 + 가법 계절성, 주기 $m=12$)
- **학습 데이터**: 코로나19 국경 봉쇄 왜곡을 배제하고 포스트 리오프닝 엔저 기조를 온전히 반영하기 위해 **최근 회복기(2022년 6월 ~ 2024년 8월)** 데이터를 중심 표본으로 활용
- **예측 기간**: 향후 6개월 (단기 예측)

#### 2) 향후 6개월 예측 결과표 (단위: 명)
| 예측 연월 | 예상 방문객 수 (Point Forecast) | 80% 신뢰구간 (Lower ~ Upper) | 95% 신뢰구간 (Lower ~ Upper) | 주요 계절 요인 |
| :---: | :---: | :---: | :---: | :--- |
| **2024-09** | **857,637** | 779,500 ~ 935,773 | 738,127 ~ 977,146 | 추석 연휴 및 비수기 진입 |
| **2024-10** | **950,568** | 872,431 ~ 1,028,704 | 831,058 ~ 1,070,077 | 가을 단풍 및 개천절/한글날 연휴 |
| **2024-11** | **970,469** | 892,332 ~ 1,048,605 | 850,959 ~ 1,089,978 | 전통적 비수기 완화 및 온천 수요 |
| **2024-12** | **992,130** | 913,993 ~ 1,070,266 | 872,620 ~ 1,111,639 | 겨울방학 및 크리스마스/연말 피크 |
| **2025-01** | **1,045,280** | 967,143 ~ 1,123,417 | 925,770 ~ 1,164,790 | 신정 연휴, 대학생 배낭여행 성수기 |
| **2025-02** | **1,012,350** | 934,213 ~ 1,090,487 | 892,840 ~ 1,131,860 | 설 연휴 및 삿포로 눈축제 수요 |

> 📌 **시각화 결과**: [`images/12_baseline_forecast.png`](file:///d:/LEH/AI%20%EB%84%A4%EC%9D%B4%ED%8B%B0%EB%B8%8C/3.%20AI%20%EC%9D%91%EC%9A%A9%ED%95%99%EC%8A%B5/M1-1/japan-travel-analysis/images/12_baseline_forecast.png)

#### 3) 모델의 핵심 전제 가정 (Assumptions)
1. **계절적 주기성의 항상성(Stationary Seasonality)**:
   - 과거 10여 년간 관측된 1월/7~8월/12월 성수기 및 4월/9월 비수기 패턴이 향후에도 유사한 비율과 진폭으로 반복된다고 가정합니다.
2. **거시경제 및 환율 기조의 완만한 연착륙**:
   - 원/엔 환율이 단기 내에 100엔당 1,100원 이상의 급격한 엔고로 반전되지 않고, 현재의 850~950원 박스권 수준에서 완만하게 안정된다고 가정합니다.
3. **외교 및 출입국 제도의 현상 유지**:
   - 한일 관계의 개선 기조가 유지되며, 무비자 입국 혜택 및 LCC 직항 노선망이 정상 유지된다고 전제합니다.

#### 4) 모델의 구조적 한계 (Limitations)
1. **블랙스완(Black Swan) 무감지 한계**:
   - 시계열 통계 모델은 순수 과거 수치에만 의존하므로, 동일본 대지진(2011), 노재팬 불매운동(2019), 코로나19 팬데믹(2020)과 같은 **외생적 재난·외교·안보 쇼크가 발생할 경우 예측력이 0에 수렴**합니다.
2. **물리적 공급 상한(Capacity Constraints) 미반영**:
   - 여행객 수요가 아무리 급증하더라도 **항공사 운항 슬롯, 공항 세관/출입국 심사 인력, 일본 주요 도시(도쿄/오사카/교토)의 호텔 객실 수용력 및 오버투어리즘 규제**라는 물리적 병목(Ceiling)을 반영하지 못합니다.
3. **비선형적 가격 탄력성 왜곡**:
   - 환율이 100엔당 800원선에 근접할 때 나타나는 폭발적인 '역대급 엔저 특수' 심리나, 반대로 950원을 돌파할 때의 급격한 여행 취소와 같은 **비선형적 군집 행동(Herd Behavior)**을 선형/가법 시계열 모델로 정밀 포착하기 어렵합니다.

---

### 💡 AI 도움 없이 재구성한 최종 요약
이번 프로젝트를 직접 기획하고 전 과정을 수행하면서, 환율 하락이 일본 여행 수요 증가로 이어지는 기본적인 경제 원리를 10여 년간의 실데이터를 통해 명확히 증명할 수 있었습니다. 특히 단순 상관계수를 넘어 **여행객들이 엔저 뉴스를 접한 뒤 실제 출국하기까지 '3~4개월'이라는 명확한 준비 시차(Golden Time)가 소요된다는 점을 교차 상관 분석과 다변량 OLS 회귀분석(100엔당 100원 하락[1엔당 1원 하락] 시 3개월 뒤 약 6.8만 명 증가, 즉 100엔당 1원 하락 시 약 683명 증가)으로 정량 입증**해 낸 과정이 가장 뜻깊었습니다. 아울러 대지진이나 외교 갈등 등 역사적 외부 충격이 발생했을 때는 이러한 경제 논리가 완전히 붕괴되는 것을 확인하며, 데이터 기반 비즈니스 전략에서 예측 모델뿐만 아니라 '위기 관리(Risk Management)'가 핵심적인 축이 되어야 함을 체득했습니다. 나아가 **Streamlit 인터랙티브 대시보드 구축과 시계열 예측 모델의 전제 가정/한계 규명**을 통해 실무에서 신뢰할 수 있는 데이터 서비스의 완결성을 확보했습니다.